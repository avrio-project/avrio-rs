...
use avrio_database::{getData, saveData, savePeerlist};
...
fn createFileStructure() -> Result<(), std::error::Error> {
    fs::create_dir_all(config().db_path + &"/blocks".to_string())?;
    fs::create_dir_all(config().db_path + &"/chains".to_string())?;
    return Ok(());
}

fn connectSeednodes(seednodes: Vec<SocketAddr>, connected_peers: Vec<SocketAddr>) -> u8 {
    let mut i: usize = 0;
...
fn firstStartUp() -> u8 {
    ...
    let e_code = saveData(
        serde_json::to_string(&genesis_block.unwrap()),
...
fn send_block() {
    ...
    if check_block(new_block) {
        ...
        if state != Err(p2pError::none) {
            // there was an error
            error!(
                "Failed to propiagte block {:?}, encountered error: {:?}",
                new_block_s.hash, state
            ); // tell them the error
            error!(
                "Block dump: non serilised {:?}, serilised {}",
                new_block, new_block_s
            ); // now flood their eyes
            process::exit(1);
        }
    }
}
